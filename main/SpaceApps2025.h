...
void train_model(std::string model) {
	...
	if (model == "K2") {
		...
		try {
			std::filesystem::path sourcePathFs(SOURCE_PATH);
			auto tessPath = sourcePathFs / "data" / "tess.csv";
			std::cout << "Loading, shuffling, and splitting TESS data..." << std::endl;
			auto datasets = dm.load_and_split_tess_data(tessPath.string(), 0.8f);
			std::cout << "Data loaded successfully." << std::endl;
			std::cout << "Training samples: " << datasets.training_inputs.size() << std::endl;
			std::cout << "Testing samples:  " << datasets.testing_inputs.size() << std::endl;
			if (datasets.training_inputs.empty()) {
			...
			DNN network(input_sequence_length, num_classes, hidden_layers,
				num_filters, filter_size, attention_dim);
			std::cout << "\nStarting training on TESS data with Hybrid CNN-Attention model..." << std::endl;
			network.train(datasets.training_inputs, datasets.training_targets, 500, 0.005f);
			std::cout << "Training complete." << std::endl;
			std::cout << "--------------------" << std::endl;
			// --- 3. Evaluate the network on the testing data ---
		...
		try {
			std::filesystem::path sourcePathFs(SOURCE_PATH);
			auto keplerPath = sourcePathFs / "data" / "kepler.csv";
			std::cout << "Loading, shuffling, and splitting Kepler data..." << std::endl;
			auto datasets = dm.load_and_split_kepler_data(keplerPath.string(), 0.8f);
			std::cout << "Data loaded successfully." << std::endl;
			std::cout << "Training samples: " << datasets.training_inputs.size() << std::endl;
			std::cout << "Testing samples:  " << datasets.testing_inputs.size() << std::endl;
			if (datasets.training_inputs.empty()) {
			...
			DNN network(input_sequence_length, num_classes, hidden_layers,
				num_filters, filter_size, attention_dim);
			std::cout << "\nStarting training on Kepler data with Hybrid CNN-Attention model..." << std::endl;
			network.train(datasets.training_inputs, datasets.training_targets, 500, 0.005f);
			std::cout << "Training complete." << std::endl;
			std::cout << "--------------------" << std::endl;
			// --- 3. Evaluate the network on the testing data ---
